...
1. 函数参数与返回值必须显式注解。
2. 变量首次赋值后类型不可变更。
3. 禁止字段名在运行时才能确定的反射调用：`getattr` / `setattr` / `delattr` 中 name 参数为变量或运行时表达式的形式。name 为字符串字面量时（如 `setattr(obj, "x", val)`）等价于 `obj.x = val`，允许使用，编译为单条 `store` 指令，速度与 C struct 写字段相同。
