...
- ✅ LLVM IR 代码生成（`int`/`float`/`bool`/`str` 函数、算术、函数调用、递归、控制流）
...
- ✅ 一元运算符：`not`（bool xor）、`-`（int sub 0 / float fneg）
- ✅ `x: T = expr` 带注解赋值（AnnAssign）编译支持，与 analyzer 对齐
- ✅ 26 个端到端用例通过（check + build）

### 已知限制（Phase 3 前不展开）
- 除法 (`/`, `//`, `%`) 尚未支持
- `break` / `continue` 尚未支持
- `str` 目前仅用于 print；字符串全局在内部与 printf 标志耦合，Phase 3 重构时拆分
